...
- 点击侧边栏顶部“新会话”或“临时会话”分区的加号，只进入临时会话输入页，不立即创建目录。
- 用户首次发送消息时，客户端才创建专属目录、注册工作区、创建会话并提交首条消息。
...
- `dsh-client-ui-conversation` 为未绑定 Session 的页面提供轻量输入框；首次发送后切换回上游标准会话输入框。
- `dsh-client-ui-workspace` 创建并注册临时工作区，并按目录路径将其会话聚合到专属分区。

当前 Harness 的未选工作区输入框与工作区列表没有足够的公开扩展槽，因此后三项通过 `patch-package` 维护。
...
2. `ConversationRoot` 的无 Session 分支及 `main.conversation` 注入契约是否变化。
...
5. 执行 `npm ci`、`npm test`、`npm run typecheck`、`npm run build`，并手工确认“输入前无目录、首次发送后有目录和分区”的完整链路。
